 ...
 *
 *  2. PHP treats a missing `opcache.file_cache` directory as a startup FATAL,
...
test('the file cache skips shared memory', function () {
    // file_cache_only keeps these CLI settings from perturbing php-fpm's own
    // shared-memory opcache, and skips SHM setup that a sub-second process
    // can never amortise.
    expect($this->ini)->toMatch('/^opcache\.file_cache_only\s*=\s*1$/m');
});

test('fpm opcache stays enabled alongside the cli settings', function () {
    // file_cache_only is a CLI-oriented setting but the ini is shared by both
    // SAPIs; FPM is long-lived and must keep its own opcache.
